...
async def _kg_get_entity(p: KgGetEntityParams, session_id: str = "") -> str:
    try:
        ...
        return await query(f"Расскажи всё что знаешь о {p.name}", mode="mix")
    except Exception as exc:
...
def _register() -> None:
    ...
    specs = [
        (
            "kg_query",
            "Поиск по графу знаний (mode: local/global/hybrid/mix)",
            KgQueryParams,
            _kg_query,
        ),
        (
            "kg_get_entity",
            "Получить все факты о сущности из графа",
            KgGetEntityParams,
            _kg_get_entity,
        ),
